...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* dc, const InitInfo& initInfo)
{
	...
	HR(CreateDDSTextureFromFile(device, mInfo.LayerMapFilename.c_str(), &texResource, &mLayerMapArraySRV));
	ReleaseCOM(texResource); // view saves reference

	HR(CreateDDSTextureFromFile(device, mInfo.BlendMapFilename.c_str(), &texResource, &mBlendMapSRV));
	ReleaseCOM(texResource); // view saves reference
}
