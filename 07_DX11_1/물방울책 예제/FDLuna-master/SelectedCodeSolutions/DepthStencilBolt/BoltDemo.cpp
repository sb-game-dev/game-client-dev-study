...
bool BlendApp::Init()
{
	...
	HR(CreateDDSTextureFromFile(md3dDevice, L"../../Assets/Textures/grass.dds", &texResource, &mGrassMapSRV));
	ReleaseCOM(texResource); // view saves reference

	HR(CreateDDSTextureFromFile(md3dDevice, L"../../Assets/Textures/water2.dds", &texResource, &mWavesMapSRV));
	ReleaseCOM(texResource); // view saves reference

	HR(CreateDDSTextureFromFile(md3dDevice, L"../../Assets/Textures/BoltAnim.dds", &texResource, &mBoltMapSRV));
	ReleaseCOM(texResource); // view saves reference
...
void BlendApp::DrawScene()
{
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		Effects::BasicFX->SetUseAnim(false);
	...
	for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		Effects::BasicFX->SetAnimArray(mBoltMapSRV);
		Effects::BasicFX->SetUseAnim(true);
		Effects::BasicFX->SetAnimIndex(mBoltFrameIndex);
