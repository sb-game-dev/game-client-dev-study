...
public:
	/* 그래픽 디바이스의 초기화. */
	/* 장치객체를 생성한다. */
	HRESULT Initialize(HWND hWnd, WINMODE isWindowed, uint32_t iWinSizeX, uint32_t iWinSizeY,
		_Out_ ComPtr<ID3D11Device>& pDevice, _Out_ ComPtr<ID3D11DeviceContext>& pContext);
...
public:
	static unique_ptr<CGraphic_Device> Create(_In_ HWND hWnd, WINMODE isWindowed, uint32_t iWinSizeX, uint32_t iWinSizeY, _Out_ ComPtr<ID3D11Device>& pDevice, _Out_ ComPtr<ID3D11DeviceContext>& pContext);
