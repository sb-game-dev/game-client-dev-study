...
void CField::Update()
{
	...
	while (true)
	{
		...
		int iResult = Battle();
		if (iResult)
			dynamic_cast<CPlayer*> (m_pPlayer)->SetHp(m_pPlayer->GetInfo().iMaxHp);
...
int CField::Battle()
{
	...
	while (true)
	{
		...
		{
		case 1:
			dynamic_cast<CPlayer*>(m_pPlayer)->TakeDamage(m_pMonster->GetInfo().iAttack);
			dynamic_cast<CMonster*>(m_pMonster)->TakeDamage(m_pPlayer->GetInfo().iAttack);
