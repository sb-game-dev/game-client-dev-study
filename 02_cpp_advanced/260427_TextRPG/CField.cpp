...
void CField::Update()
{
	...
	while (true)
	{
		...
		case 2:
			...
			break;
		case 3:
			m_pMonster = new CMonster("고급", 60, 9);
			break;
		case 4:
			return;
		default:
...
int CField::Battle()
{
	...
	while (true)
	{
		...
		{
		case 1:
			if (pTempPlayer) pTempPlayer->TakeDamage(m_pMonster->GetInfo().iAttack);
			if (pTempMonster) pTempMonster->TakeDamage(m_pPlayer->GetInfo().iAttack);
