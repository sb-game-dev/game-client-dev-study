...
void CField::Update()
{
	...
	while (true)
	{
		...
		{
		case 1:
			m_pMonster = new CMonster("초급", 30 * iInput, 3 * iInput);
			break;
		case 2:
			m_pMonster = new CMonster("중급", 30 * iInput, 3 * iInput);
			break;
		case 3:
			m_pMonster = new CMonster("고급", 30 * iInput, 3 * iInput);
			break;
		case 4:
			return;
		default:
...
void CField::Release()
{
	SAFE_DELETE(m_copyPlayer);
	SAFE_DELETE(m_pMonster);
}

void CField::SetPlayer(CPlayer* pPlayer)
{
	m_copyPlayer = pPlayer;
}
