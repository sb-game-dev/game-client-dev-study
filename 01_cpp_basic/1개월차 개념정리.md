...
### 10일차 (260330) 디버깅, 함수포인터, 문자배열, 문자열 포인터
...
### 11일차 (260402) 문자열함수, 사용자 정의 자료형, 구조체, 공용체
<details>
...
### 12일차 (260403) 열거체, 동적 할당, 메모리 함수


<details>
	<summary>열거체(enum)</summary>

- 열거체란?: 상수를 한 번에 여러 개 만드는 문법  
- 상수값을 정해주지 않으면 첫 번째 원소가 자동으로 0초기화 되고 그 이후로 원소마다 하나씩 증가한 값을 가지게 된다.  
- 상수의 개수와 상관 없이 메모리 크기는 4byte(int와 같음)  
- 배열 또는 switch와 연계하여 사용됨.  
...
int main()
{
	Number eNumber = FOUR;
	cout<< eNumber <<endl;
	cout<< FOUR; // 보통 이렇게 맣이 사용함.

	int iRes = ONE + TWO; //자동으로 형 변환함.
	cout<< iRes << endl; 
}
```
</details>
...
<details>
	<summary>stack영역에 저장해서 발생하는 오류 예시</summary>
	
```cpp
char* GetString()
{
	char szName[32] = ""; // stack에 저장되기 때문에 함수 종료 후 메모리는 사라짐. 만약 전역변수였다면 괜찮음.
	cin >> szName;
	return szName;
}
int main()
{
	char* pName = GetString(); //사라진 메모리를 가르킴(댕글링 포인터)
	cout << pName << endl;
}
```
</details>
...
<details>
	<summary>동적 할당</summary>

- 동적 할당이란?: 런 타임에 메모리를 예약하는 행위, heap 영역에 해당하는 메모리를 사용하기 위한 행위  
- 반드시 포인터가 필요함.(heap영역은 오로지 주소 참조를 통해서만 접근이 가능하다)  
- 사용자가 필요한 타임에 메모리를 할당하고 필요하지 않을 때, 메모리를 반환 할 수 있는 자유가 생긴다.  
- heap 영역을 할당 / 해제 하기 위해서는 논리적인 판단에 의해 프로그래머가 결정해야 한다.
- 현재는 window 자체에서 시간이 지나도 사용하지 않으면 메모리를 가져가지만 그래도 해야 함.
</details>

<details>
	<summary>c언어 동적 할당</summary>

- 함수를 이용하여 동적 할당(malloc, calloc)  
> malloc: heap영역에 매개변수의 크기만큼의 메모리를 할당하고 그중 첫 번째 주소를 반환시키는 함수
```cpp
int* pSrc = (int*)malloc(sizeof(int));  //malloc의 반환값이 void* 이기 때문에(int*)로 초기화 함.
int* pArr = (int*)malloc(sizeof(int) * 4); // int 4개를 저장할 수 있는 연속된 메모리 공간(배열)을 힙에 할당 4대신 변수를 사용하여 런타임에 배열의 크기를 정할 수 있음
```
> calloc: heap영역에 매개변수의 크기만큼의 메모리를 할당하고 그중 첫 번째 주소를 반환시키는 함수. 메모리의 개수를 정할 수 있음.
```cpp
int* pSrc = (int*)calloc(1, sizeof(int));  //malloc의 반환값이 void* 이기 때문에(int*)로 초기화 함.
int* pArray = (int*)calloc(4, sizeof(int));
// int 4개를 저장할 수 있는 연속된 메모리 공간(배열)을 힙에 할당 4대신 변수를 사용하여 런타임에 배열의 크기를 정할 수 있음
```
</details>

<details>
	<summary>c언어 동적 할당 반환</summary>

- free: 메모리 반환 신호 발송, 포인터가 지닌 주소를 삭제하는 기능은 없음
- 따라서 free 명령어 사용 이후 포인터를 NULL로 초기화해야함.
```cpp
int* p = (int*)malloc(4);
free(p);		
p = NULL;		//free 사용 후 반드시 NULL로 초기화
```
</details>

<details>
	<summary>c++ 동적 할당</summary>

- new 키워드 사용. 선언과 동시에 초기화 가능.
```cpp
int* p = new int(10);
cout<<p<<endl;
cout<<*p<<endl;

int* pArr = new int[4]{}; //4 대신 변수를 사용하면 런타임에 배열의 크기를 지정할 수 있음.
cout<<pArr<<end;
for(int i=0;i<4;i++)
	cout<<pArr[i]<<endl;
```
...
<details>
	<summary>c++ 동적 할당 반환</summary>

- delete 키워드 사용. 사용 후에 nullptr로 초기화 해야 함.
```cpp
int* p = new int(10);
cout<<p<<endl;
cout<<*p<<endl;
...
<details>
	<summary>메모리 함수</summary>

> memset: 사용자가 원하는 값으로 '바이트 단위'로 값을 채워주는 함수. 0또는 -1로 초기화 할때만 사용  
```cpp
...
// 복사 받을 공간의 주소, 복사할 데이터의 주소, 복사 받을 메모리 크기 => sizeof() * 갯수

memcpy(iTemp, iArray, sizeof(iTemp) * 5);
memmove(iTemp, iArray, sizeof(iTemp) * 5); // 중간 버퍼를 한 번 거치고 복사함
...
### 13일차 (260406) 전처리기 명령어, 입출력(콘솔 문자)

<details>
	<summary>전처리기 명령어</summary>

> 매크로

- 코드를 강제 치환 하는 명령어.
- 매크로 함수 사용시 괄호를 적절히 사용해야 함.
- 여러 줄을 사용할 때는 \를 마지막줄을 제외하고 써야 함.
- 여러 줄을 사용하지 않고 한 줄로 사용할 수 있음

> 조건부 컴파일

- 조건에 따라 전처리기 시점에 컴파일 여부를 결정할 수 있음.
- 주로 include guard로 사용하거나 디버그모드, 릴리즈모드 변경할 때 사용하거나, 운영체제를 변경할 때 사용함

> #if

- #if		조건		: 조건이 참이면 컴파일  
- #elif		조건		: 조건이 참이면 컴파일  
- #else					: 위의 조건이 모두 거짓이면 컴파일  
...
``` 
> #ifdef
- #ifdef  전처리기 정의 여부 : 만약 어떤 매크로가 정의 되어 있다면 컴파일
- #elif   전처리기 정의 여부 : 만약 어떤 매크로가 정의 되어 있다면 컴파일
- #else						: 위의 전처리기들이 정의 되어 있지 았다면 컴파일
- #endif 					: 조건부 컴파일을 종료하겠다는 의미
```cpp
#ifdef _DEBUG
	cout << "디버그 모드" << endl;
...
```
> #ifndef
- #ifndef  전처리기 정의 여부 : 만약 어떤 매크로가 정의 되어 있지 않다면 컴파일
- #elif    전처리기 정의 여부 : 만약 어떤 매크로가 정의 되어 있지 않다면 컴파일
- #else	위의 전처리기들이 정의 되어 있다면 컴파일
- #endif // 조건부 컴파일을 종료하겠다는 의미

</details>

<details>
	<summary>콘솔 문자 입출력 </summary>

- 콘솔 입출력은 프로그램 시작 시 표준 스트림이 열려있음.
- 따라서 굳이 안만들어도 됨. 
...
| stdout    | 표준 출력 스트림              |      |
| stderr    | 표준 에러 출력 스트림         |      |
| getchar   | stdin에서 1글자 입력 (개행 포함) | int c = getchar(); |
| putchar   | stdout으로 1글자 출력         | putchar(c); |
| fgetc     | 스트림에서 1글자 입력 (개행 포함) | int c = fgetc(stdin); |
| fputc     | 스트림으로 1글자 출력         | fputc(c, stdout); |
| EOF       | 입력 끝 또는 오류 (-1)        | if(c == EOF) break; |

- getchar, fgetc 둘 다 한 글자만 가져감
- 따라서 여러 개를 입력 받을 때 버퍼를 비우지 않으면  개행문자 또는 공백이 섞여있으면 잘못 입력 받을 수 있음.
- cin만 쓸 때는 상관 없지만 cin과 섞어서 쓸 때는 문제가 생김. 
```cpp
int a, b, c;
cin >> a >> b >> c;
...
### 14일차 (260407) 입출력(콘솔 문자열, 파일), 파일 함수, 레퍼런스 자료형

<details>
	<summary>입출력(콘솔 문자열)</summary>

| 함수     | 문법 예시                                    | 공백 포함 | 개행 포함 |
|----------|-----------------------------------------------|----------|----------|
| puts     | puts(szName);                                 | O        | 자동 추가 |
| gets_s   | gets_s(szName, sizeof(szName));               | O        | 제거됨    |
| fputs    | fputs(szName, stdout);                        | O        | 없음      | 
| fgets    | fgets(szName, sizeof(szName), stdin);         | O        | 포함됨    | 

- fgets를 사용할 때 버퍼에 남이있는 개행문자('\n') 처리 방법
```cpp
char szID[9] = "";
char szName[16] = "";
...
<details>
	<summary>입출력(파일)</summary>

> 파일 입출력의 흐름
1. 파일 개방(스트림 생성)  
2. 파일 읽기 또는 파일 쓰기 함수 호출
3. 개방한 파일 소멸
...
> 1. 파일 개방
- fopen_s(개방할 스트림 주소를 저장할 포인터, 파일의 경로, 파일의 모드)  
- 파일의 경로는 절대 경로를 사용하거나 현재 프로젝트를 기준으로하는 상대 경로 두 가지가 있음.
```cpp
errno_t err = fopen_s(&pFile, "../Data/Test.txt", "wt");
//errono_t : int와 같지만, "에러 코드를 담는 용도"라고 의미를 붙인 타입
//c언어 방식이라 참이면 0이다.
```
...
```cpp
FILE* pReadFile = NULL;
errno_t errRead = fopen_s(&pReadFile, "../Data/Test.txt", "rt");
...
<details>
	<summary>파일 함수</summary>
	
> fseek : 파일 내 커서의 위치를 이동시키는 용도의 함수

- fseek(개방한 파일 스트림, 이동할 바이트 수, 커서의 시작 지점);
- 파일 개방시 텍스트모드로 개방하면 개행문자 때문에 오류가 발생할 수 있음.
- 따라서 바이너리모드로 개방하는 것이 좋음.   
- 커서의 시작지점 옵션

| 옵션      | 기준 위치   | 설명                        | 예시 코드                       | 결과                  |
|-----------|------------|-------------------------------|---------------------------------|-----------------------|
| SEEK_SET  | 파일 시작  | 파일의 맨 처음 기준으로 이동  | fseek(pFile, 0, SEEK_SET);       | 처음 위치로 이동      |
| SEEK_CUR  | 현재 위치  | 현재 커서 위치 기준으로 이동  | fseek(pFile, 5, SEEK_CUR);       | 현재 위치에서 +5 이동 |
| SEEK_END  | 파일 끝    | 파일의 끝 기준으로 이동       | fseek(pFile, 0, SEEK_END);       | 파일 끝으로 이동      |

```cpp
FILE* pLoad = NULL;

errno_t err = fopen_s(&pLoad, "../Data/Test.txt", "rt");

if (0 == err)
{
	fseek(pLoad, 3, SEEK_SET);	// 커서의 시작 지점으로 부터 3바이트만큼 이동시켜라
	
	char cName = fgetc(pLoad);

	cout << "현재 커서가 가리키는 영어 : " << cName << endl;
	cout << "현재 커서가 있는 위치 : " << ftell(pLoad) << endl;

	fclose(pLoad);
}
```
	
> ftell : 커서의 위치를 알려주는 함수, 첫 시작 위치는 0부터 시작
```cpp
//파일의 크기 구하는 예시
...
> foef : 커서가 eof에 도달했는 검사하는 함수, eof에 도달했다면 0이 아닌 값(-1)을 반환
```
FILE* pLoad = NULL;
...
<details>
	<summary>파일 입출력 함수 </summary>
> 읽기 함수
```cpp

size_t fread(void* buffer, size_t size, size_t count, FILE* stream);
// buffer: 읽고나서 반환받을 할 변수의 **주소**
// size: 읽고나서 저장 할 변수의 메모리 크기(sizeof() 함수 사용)
// count: 변수의 갯수(배열일 경우 배열의 크기)
// stream: 개방한 스트림
...
// buffer: 코드내에 있는 변수 중 파일에 저장할 변수의 **주소**
// size: 저장할 변수의 자료형 메모리 크기(sizeof() 함수 사용)
// count: 변수의 갯수
// stream: 개방한 스트림 
```
</details>
...
- **직접 참조**하기 위한 자료형  
- 용량을 갖지 않는다.  
- 불필요한 연산자를 사용할 필요가 없다.  

> 레퍼런스 규칙
1. 레퍼런스는 반드시 선언과 동시에 초기화 해야 한다.  
2. 레퍼런스는 오직 하나의 데이터만 참조 가능하다.
3. 레퍼런스는 L-value만 참조가 가능하다.(단, const& 를 사용하면 r-value를 읽기 전용으로 참조 가능)

```cpp
int iData = 10;

int* p = &iData;	// 주소값 (간접)참조
*p = 20;

int& r = iData;		// 주소값 (직접)참조
